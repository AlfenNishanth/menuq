  ...
  return (
    <div className={`flex items-center justify-center min-h-screen transition-colors duration-500 ${darkMode ? 'bg-gradient-to-r from-indigo-900 via-purple-900 to-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <div className={`p-8 rounded-2xl shadow-xl w-96 transform hover:scale-105 transition-transform duration-300 backdrop-blur-lg ${darkMode ? 'bg-gray-800 bg-opacity-30' : 'bg-white'}`}>
        <h2 className="text-3xl font-extrabold mb-6 text-center">Your MenuQ Awaits</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-between items-center">
            <label className="flex items-center cursor-pointer">
              <input type="checkbox" className="hidden peer" checked={rememberMe} onChange={() => setRememberMe(!rememberMe)} />
              <span className="ml-2 text-sm peer-checked:text-purple-600">Remember Me</span>
            </label>
            <button type="button" className="text-sm text-purple-400 underline hover:text-purple-600" onClick={handleResetPassword}>
              Forgot Password?
            </button>
          </div>
        ...
        <div className="flex justify-center mt-4 text-gray-400">
          New here?
          <Link to="/signup" className="text-purple-400 underline ml-2 hover:text-purple-600 transition">Create an Account</Link>
        </div>
      </div>
